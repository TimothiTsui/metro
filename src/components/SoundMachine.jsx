...
class SoundMachine extends Component {
  ...
  componentWillUnmount() {
    // Stop and dispose any Tone.js objects here if needed
    this.transport.stop();
  ...
  setTimeSignature(timeSignature) {
	if (timeSignature !== this.transport.timeSignature) {
	  ...
	  this.transport.scheduleOnce(() => {
		this.transport.scheduleRepeat(
		  (time) => this.repeat(time, this.state.track),
		  "1m",
		  timeSignature - 1
		);
	  }, 0);
	...
	this.setState({ timeSignature: timeSignature });
  }
                ...
                <Row>
                  <Col>
                    <h2>
                      <span
                        ref="bpmInfo"
                        className="badge badge-dark bpm-info"
                      >
